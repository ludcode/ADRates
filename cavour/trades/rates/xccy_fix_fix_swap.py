...
"""
Cross-Currency Fixed-Fixed Swap implementation.

Provides the XccyFixFix class for creating and valuing cross-currency swaps
where two fixed legs in different currencies are exchanged. Includes notional
exchanges at start and maturity.

Key features:
- Fixed leg in domestic currency (pays fixed coupon)
- Fixed leg in foreign currency (pays fixed coupon)
- Discounting using cross-currency discount curve for foreign leg
- FX conversion for consistent domestic currency valuation

The XCCY fixed-fixed swap structure:
- Start date: XCCY spot date (T_spot = value date + FX spot lag)
- Domestic fixed leg (pays/receives fixed coupon)
- Foreign fixed leg (pays/receives fixed coupon) with independent calendar
- Initial notional exchange at T_spot
- Final notional exchange at maturity

Example:
    >>> # Create a 5Y USD/GBP fixed-fixed swap
    >>> # Domestic (GBP): pay fixed 4.5%
    >>> # Foreign (USD): receive fixed 5.2%
    >>> value_dt = Date(15, 6, 2023)
    >>> xccy_swap = XccyFixFix(
    ...
    ...     foreign_coupon=0.052,  # 5.2% fixed
...
class XccyFixFix:
    """
    Cross-currency fixed-fixed swap with notional exchange.

    A fixed-fixed swap exchanges fixed rate payments in domestic currency
    against fixed rate payments in foreign currency. Notional amounts are
    exchanged at both the start and maturity of the swap.

    Valuation:
    - Domestic fixed leg: discounted with domestic curve
    - Foreign fixed leg: discounted with XCCY curve
    ...
    def __init__(self,
        ...
        """
        Create a cross-currency fixed-fixed swap.
        ...
        Args:
            ...
            foreign_coupon: Fixed coupon rate for foreign leg (decimal, e.g., 0.052 for 5.2%)
            domestic_freq_type: Payment frequency for domestic fixed leg
            foreign_freq_type: Payment frequency for foreign fixed leg
            domestic_dc_type: Day count convention for domestic fixed leg
            foreign_dc_type: Day count convention for foreign fixed leg
            domestic_floating_index: Index curve type for domestic currency (for discounting)
            foreign_floating_index: Index curve type for foreign currency (for discounting)
    ...
    def value(self,
              ...
              spot_fx: float):
        """
        Value the cross-currency fixed-fixed swap.

        Args:
            value_dt: Valuation date
            domestic_discount_curve: Domestic OIS discount curve
            foreign_discount_curve: Foreign OIS discount curve (not used for fixed-fixed)
        ...
        # Value foreign fixed leg using XCCY curve
        foreign_leg_value = self._foreign_leg.value(
            value_dt=value_dt,
            discount_curve=xccy_discount_curve
        )

        # Add notional exchanges for foreign fixed leg (manual handling)
        # Foreign leg type is opposite of domestic
        foreign_leg_type = SwapTypes.PAY if self._domestic_leg_type == SwapTypes.RECEIVE else SwapTypes.RECEIVE

        if self._effective_dt >= value_dt:
            df_start = xccy_discount_curve.df(self._effective_dt)
            start_exchange_pv = -self._foreign_notional * df_start
            # Apply leg type sign
            if foreign_leg_type == SwapTypes.RECEIVE:
                foreign_leg_value += start_exchange_pv
            else:
                foreign_leg_value -= start_exchange_pv

        if self._maturity_dt >= value_dt:
            df_end = xccy_discount_curve.df(self._maturity_dt)
            end_exchange_pv = self._foreign_notional * df_end
            # Apply leg type sign
            if foreign_leg_type == SwapTypes.RECEIVE:
    ...
    def print_valuation(self):
        ...
        print("\n" + "="*80)
        print("FOREIGN FIXED LEG VALUATION:")
        print("="*80)
