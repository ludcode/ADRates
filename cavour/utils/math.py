...
def accrued_interpolator(t_set: float,  # Settlement time in years
    ...
    raise LibError("Failed to calculate accrued")
...
def test_range(x: np.ndarray,
    ...
    for i in range(0, len(x)):
        if x[i] < lower:
            raise LibError("Value below lower.")
        if x[i] > upper:
            raise LibError("Value above upper.")
...
def norminvcdf(p):
    ...
    if p < 0.0 or p > 1.0:
        raise LibError("p must be between 0.0 and 1.0")
...
#    if abs(r) > 0.9999999:
#        raise LibError("Phi2: |Correlation| > 1")
