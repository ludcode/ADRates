...
class Date():
    ...
    def __init__(self, d, m, y, hh=0, mm=0, ss=0):
        ...
        if y < 1900:
            raise LibError("Year cannot be before 1900")
        ...
        if d < 1:
            raise LibError("Date: Leap year. Day not valid.")
        ...
        if leap_year:
            ...
            if d > month_days_not_leap_year[m - 1]:
                print(d, m, y)
                raise LibError("Date: Not Leap year. Day not valid.")

        if hh < 0 or hh > 23:
            raise LibError("Hours must be in range 0-23")

        if mm < 0 or mm > 59:
            raise LibError("Minutes must be in range 0-59")

        if ss < 0 or ss > 59:
            raise LibError("Seconds must be in range 0-59")
    ...
    def add_hours(self, hours):
        ...
        if hours < 0:
            raise LibError("Number of hours must be positive")
    ...
    def add_weekdays(self,
        ...
        if isinstance(num_days, int) is False:
            raise LibError("Num days must be an integer")
    ...
    def add_months(self,
        ...
        for i in range(0, num_months):
            ...
            if int(mmi) != mmi:
                raise LibError("Must only pass integers or float integers.")
    ...
    def third_wednesday_of_month(self,
        ...
        raise LibError("Third Wednesday not found")
    ...
    def add_tenor(self,
        ...
        if isinstance(tenor, list) is True:
            ...
            for ten in tenor:
                if isinstance(ten, str) is False:
                    raise LibError("Tenor must be a string e.g. '5Y'")
        else:
        ...
        for tenor_str in tenor:
            ...
            if tenor_str == "ON":   # overnight - should be used only if spot days = 0
                ...
                raise LibError("Unknown tenor type in " + tenor)
    ...
    def __repr__(self):
        ...
        if g_date_type_format == DateFormatTypes.UK_LONGEST:
            ...
            raise LibError("Unknown date format")
...
def days_in_month(m, y):
    ...
    if m < 1 or m > 12:
        raise LibError("Month must be 1-12")
