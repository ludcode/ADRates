...
import pandas as pd
from typing import List, Tuple, Iterator, Dict, Iterable, Optional
...
class Risk:
    """
    Container for multiple per-curve Delta ladders.
    Allows access like:
        risk_report.SONIA.value
        risk_report.SONIA.ladder
    """
    def __init__(self, deltas: Iterable[Delta]):
        self._by_curve = {}  # type: Dict[str, Delta]
        for d in deltas:
            name = d.curve_type.name
            if name in self._by_curve:
                raise ValueError(f"Duplicate curve {name}")
            self._by_curve[name] = d
            # expose as attribute
            setattr(self, name, d)

    def __repr__(self):
        parts = []
        for name, d in self._by_curve.items():
            mv = d.value
...
class AnalyticsResult:
    ...
    def __init__(
        ...
        gamma: Optional[jnp.ndarray] = None, #still need to develop it
    ):
