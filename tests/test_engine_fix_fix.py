"""
Test Engine for XccyFixFix (Fixed-Fixed Cross-Currency Swaps)
...
def test_engine_fix_fix_value():
    """Test engine VALUE for XccyFixFix swap against direct valuation.

    Creates a GBP/USD fixed-fixed swap with:
    - Domestic (GBP) fixed leg: pay 4.6%
    - Foreign (USD) fixed leg: receive 5.3%
    ...
    # Create fixed-fixed swap: pay GBP 4.6% fixed, receive USD 5.3% fixed
    fix_fix_swap = XccyFixFix(
        ...
        domestic_coupon=0.046,  # 4.6% fixed
        foreign_coupon=0.053,  # 5.3% fixed (receive)
    ...
    direct_value = fix_fix_swap.value(
        value_dt=value_dt,
        domestic_discount_curve=gbp_curve,
        foreign_discount_curve=usd_curve,  # Not used for fixed legs
        xccy_discount_curve=usd_curve,  # Using USD curve as XCCY curve for now
        spot_fx=spot_fx
    )

    # Get leg PVs for diagnostics
    domestic_leg_value = fix_fix_swap._domestic_leg.value(
        value_dt=value_dt,
        discount_curve=gbp_curve
    )
    foreign_leg_value = fix_fix_swap._foreign_leg.value(
        value_dt=value_dt,
        discount_curve=usd_curve  # Using USD curve as XCCY curve
    )

    print(f"Direct valuation leg PVs:")
    print(f"  Domestic (fixed): {domestic_leg_value:,.2f} GBP")
    print(f"  Foreign (fixed): {foreign_leg_value:,.2f} USD")
    print()

    # Engine valuation
    engine = Engine(model)
    result = engine.compute(fix_fix_swap, [RequestTypes.VALUE])
    ...
    print("=" * 80)
    print("ENGINE FIXED-FIXED SWAP VALUE TEST:")
    print("=" * 80)
    ...
    print("Engine FIXED-FIXED SWAP VALUE test passed!")


def test_engine_fix_fix_delta_finite_diff():
    """Test engine DELTA for XccyFixFix using finite difference validation.
    ...
    def build_model_and_swap(gbp_rates, usd_rates):
        ...
        swap = XccyFixFix(
            ...
            domestic_coupon=0.046,  # 4.6% fixed
            foreign_coupon=0.053,  # 5.3% fixed
            domestic_freq_type=FrequencyTypes.ANNUAL,
            foreign_freq_type=FrequencyTypes.ANNUAL,
            domestic_dc_type=DayCountTypes.ACT_365F,
            foreign_dc_type=DayCountTypes.ACT_360,
    ...
    model_base, fix_fix_swap_base = build_model_and_swap(gbp_rates_base, usd_rates_base)

    # Compute DELTA using engine
    engine_base = Engine(model_base)
    result_base = engine_base.compute(fix_fix_swap_base, [RequestTypes.VALUE, RequestTypes.DELTA])
    ...
    print("=" * 80)
    print("ENGINE FIXED-FIXED SWAP DELTA TEST (Finite Difference Validation)")
    print("=" * 80)
    print()
    ...
    tolerance = 10.0  # 10.0 GBP absolute tolerance per pillar (looser for fixed-fixed due to curve reconstruction effects)
    ...
    print("Engine FIXED-FIXED SWAP DELTA test passed!")
    print("=" * 80)
